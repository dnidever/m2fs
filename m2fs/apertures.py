...
def id_lines_template(arctempfile):
    #with open(directory+'arc_templates') as f:
    with open(arctempfile) as f:
        data = f.readlines()
    arcfilename = []
    arcfiltername = []
    for line in data:
    ...
    for i in range(0,len(arcfilename)):
        ...
        id_lines_template0 = m2fs.get_id_lines_template(extract1d_array[0],linelist,id_lines_continuum_rejection_low,
                                                        id_lines_continuum_rejection_high,id_lines_continuum_rejection_iterations,
                                                        id_lines_continuum_rejection_order,id_lines_threshold_factor,id_lines_window,
                                                        id_lines_order,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_sigma,
...
def id_lines_check(idcheckfile):
    #with open(directory+'id_lines_check') as f:
    with open(idcheckfile) as f:
        data = f.readlines()
    arcfilename = []
    arcfiltername = []
    for line in data:
    ...
    for i in range(0,len(arcfilename)):
        ...
        id_lines_check0 = m2fs.get_id_lines_template(extract1d_array[0],linelist,id_lines_continuum_rejection_low,id_lines_continuum_rejection_high,
                                                     id_lines_continuum_rejection_iterations,id_lines_threshold_factor,id_lines_window,id_lines_order,
                                                     id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_sigma,id_lines_tol_angs,
                                                     id_lines_template_fiddle,id_lines_array[0],resolution_order,resolution_rejection_iterations)
...
def initialize():
    ...
    if (not(columnspec_array_exists))|(columnspec_array_exists & overwrite):
        ...
        data = astropy.nddata.CCDData.read(data_file)  # format is such that data.data[:,0] has column-0 value in all rows
        columnspec_array = m2fs.get_columnspec(data,trace_step,n_lines,columnspec_continuum_rejection_low,columnspec_continuum_rejection_high,
                                               columnspec_continuum_rejection_iterations,columnspec_continuum_rejection_order,threshold_factor,window)
        '''
...
def trace_all(a):
    ...
    if (not(aperture_array_exists))|(aperture_array_exists & overwrite):
        ...
        for j in range(0,len(apertures_profile_middle.fit)):
            aperture_array.append(m2fs.get_aperture(j,columnspec_array,apertures_profile_middle,middle_column,trace_order,trace_rejection_sigma,
...
def trace_edit(a):
    ...
    j = np.long(command)-1
    aperture_array[j] = m2fs.get_aperture(j,columnspec_array,apertures_profile_middle,middle_column,trace_order,trace_rejection_sigma,
                                          trace_rejection_iterations,image_boundary,trace_shift_max,trace_nlost_max,
                                          profile_rejection_iterations,profile_nsample,profile_order,window)
...
def scatteredlightcorr(a):
    for j in range(0,len(allfile0[i])):
        ...
        if (not(scatteredlightcorr_exists))|(scatteredlightcorr_exists & overwrite):
            ...
            scatteredlightfunc = m2fs.get_scatteredlightfunc(data,apmask0,scatteredlightcorr_order,
                                                             scatteredlightcorr_rejection_iterations,scatteredlightcorr_rejection_sigma)
            y,x = np.mgrid[:len(data.data),:len(data.data[0])]
            scattered_model = CCDData(scatteredlightfunc.func(x,y)*u.electron,mask=np.full((len(data.data),len(data.data[0])),False,dtype=bool),
                                      uncertainty=StdDevUncertainty(np.full((len(data.data),len(data.data[0])),scatteredlightfunc.rms,dtype='float')))
        ...
        if (not(extract1d_array_exists))|(extract1d_array_exists & overwrite):
            ...
            for k in range(0,len(aperture_array)):
                ...
                if apertures_profile_middle.realvirtual[k]:
                    extract1d0 = m2fs.get_extract1d(k,scatteredlightcorr0,apertures_profile_middle,aperture_array,
                                                    aperture_peak,pix,extract1d_aperture_width)
            ...
            for k in range(0,len(aperture_array)):
                ...
                if apertures_profile_middle.realvirtual[k]:
                    extract1d0 = m2fs.get_extract1d(k,scatteredlightcorr0,apertures_profile_middle,
                                                    aperture_array,aperture_peak,pix,extract1d_aperture_width)
                    if len(np.where(extract1d0.spec1d_mask==False)[0])>0:
                        if np.max(extract1d0.spec1d_flux)>0.:
                            extract = extract1d0
                extract1d_array.append(extract)
    ...
    for j in range(0,len(tharfile0[i])):
        ...
        if (not(id_lines_array_exist))|(id_lines_array_exist & overwrite):
            ...
            for j in range(0,len(extract1d_array)):
                ...
                id_lines_array.append(m2fs.get_id_lines_translate(extract1d_array_template[0],id_lines_template0,extract1d_array[j],
                                                                  linelist,id_lines_continuum_rejection_low,id_lines_continuum_rejection_high,
                                                                  id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_order,
                                                                  id_lines_threshold_factor,id_lines_window,id_lines_order,
                                                                  id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_sigma,
                                                                  id_lines_tol_angs,id_lines_tol_pix,resolution_order,resolution_rejection_iterations,
                                                                  id_lines_translate_add_lines_iterations))
...
def tharcheck(a):
    ...
    if (not(thars_array_exists))|(thars_array_exists & overwrite):
        ...
        for q in range(0,len(thars_array)):
            cb1 = ax1.scatter(thars_array[q].wav,np.full(len(thars_array[q].wav),thars_array[q].aperture),
                              c=thars_array[q].pix_std,s=3,cmap='inferno',vmin=0,vmax=1.0)
            cb2 = ax2.scatter(thars_array[q].wav,np.full(len(thars_array[q].wav),thars_array[q].aperture),
                              c=thars_array[q].vel_func_std,s=3,cmap='inferno',vmin=0,vmax=1.0)
            for abc in range(0,len(thars_array[q].pix)):
                ax3.plot(thars_array[q].temperature-thars_array[q].temperature[0],
                         thars_array[q].pix[abc]-thars_array[q].pix[abc][0],alpha=0.3)
    ...
    for j in range(0,len(scifile0[i])):
        ...
        if (not(cr_reject_array_exists))|(cr_reject_array_exists & overwrite):
            ...
            for k in range(0,len(extract1d_array)):
                cr_reject_array.append(m2fs.get_cr_reject(extract1d_array[k],cr_rejection_low,cr_rejection_high,cr_rejection_order,
                                                          cr_rejection_iterations,cr_rejection_tol,cr_rejection_collateral))
...
def throughputcorr(a):
    ...
    for j in range(0,len(allfile0[i])):
        if ((allfile0[i][j] not in tharfile0[i])&(allfile0[i][j] not in flatfile0[i])):  # no need to do this for thars or flats
            ...
            #### need to turn these into mjd-weighted averages!!!  Also, in get_throughputcorr, need to change the penalty for not having len(wav)>100
            ...
            if (not(throughputcorr_array_exists))|(throughputcorr_array_exists & overwrite):
                ...
                if len(keep)>0:
                    ...
                    for k in range(0,len(cr_reject_array)):
                        twilightstack_array0.append(m2fs.extract1d(aperture=cr_reject_array[k].aperture,spec1d_pixel=cr_reject_array[k].spec1d_pixel,
                                                                   spec1d_flux=np.full(len(cr_reject_array[k].spec1d_pixel),0.,dtype='float'),
                                                                   spec1d_uncertainty=StdDevUncertainty(np.full(len(cr_reject_array[k].spec1d_pixel),999.,dtype='float')),
                                                                   spec1d_mask=np.full(len(cr_reject_array[k].spec1d_pixel),True,dtype='bool')))
                    ...
                    for k in range(0,len(cr_reject_array)):
                        if ((len(np.where(cr_reject_array[k].spec1d_mask==False)[0]>0))&(len(wavcal_array[k].wav)>0)):
                            ...
                            for z in range(0,len(twilightstack_array_array)):
                                this0 = np.where(np.array([twilightstack_array_array[z][zz].aperture \
                                                           for zz in range(0,len(twilightstack_array_array[z]))])==cr_reject_array[k].aperture)[0]
                                this.append(this0)
                            ...
                            for q in range(0,len(cr_reject_array[k].spec1d_flux)):
                                if cr_reject_array[k].spec1d_mask[q]==False:
                                    ...
                                    for z in range(0,len(twilightstack_array_array)):
                                        if len(this[z])>0:
                                            if ((len(twilightstack_wavcal_array_array[z][this[z][0]].wav)>0) & 
                                                (len(np.where(twilightstack_array_array[z][this[z][0]].spec1d_mask==False)[0])>0)):
                                                weight = 1.
#                                                weight = 1./np.abs(twilightstack_array_array_mjd[z]-mjd_mid)
                                                ...
                                                sum1 += weight*np.interp(wavcal_array[k].wav[q],tz.wav[tz.spec1d_mask==False],
                                                                         tz.spec1d_flux.value[tz.spec1d_mask==False])
                                                sum2 += weight
                                                use = True
                                    if use:
...
#                                        print(sum1,sum2,weight,twilightstack_array_array_mjd[z],mjd_mid)
                    ...
                    twilightstack_continuum_array,throughput_continuum = m2fs.get_throughput_continuum(twilightstack_array0,wavcal_array,
                                                                                                       throughputcorr_continuum_rejection_low,
                                                                                                       throughputcorr_continuum_rejection_high,
                                                                                                       throughputcorr_continuum_rejection_iterations,
                                                                                                       throughputcorr_continuum_rejection_order)
                    ...
                    for k in range(0,len(cr_reject_array)):
                        ...
                        if ((len(np.where(twilightstack_array0[k].spec1d_mask==False)[0])>0)&(len(wavcal_array[k].wav)>0)):
                            throughput,throughputcorr = m2fs.get_throughputcorr(k,throughput_continuum,cr_reject_array,twilightstack_array0,
...
#                id_nlines = np.array([len(np.where(np.array(thar[0][q].wav)>0.)[0]) for q in range(0,len(thar[0]))],dtype='int')
                ...
                for k in range(0,len(throughputcorr_array)):
                    ...
                    sky_array.append(m2fs.extract1d(spec1d_pixel = throughputcorr_array[k].spec1d_pixel*u.AA,spec1d_flux=zero,
                                                    spec1d_mask=throughputcorr_array[k].spec1d_mask))
        ...
        if (not(sky_target_check_exists))|(sky_target_check_exists & overwrite):
            ...
            for k in range(0,len(skies)):
                if len(stack_wavcal_array[skies[k]].wav)>0:
                    ax1.plot(stack_wavcal_array[skies[k]].wav[stack_array[skies[k]].spec1d_mask==False],
                             stack_array[skies[k]].spec1d_flux[stack_array[skies[k]].spec1d_mask==False],lw=0.3,alpha=0.2)
            for k in range(0,len(targets)):
                if len(stack_wavcal_array[targets[k]].wav)>0:
                    ax2.plot(stack_wavcal_array[targets[k]].wav[stack_array[targets[k]].spec1d_mask==False],
                             stack_array[targets[k]].spec1d_flux[stack_array[targets[k]].spec1d_mask==False],lw=0.3,alpha=0.2)
            ...
            for k in range(0,len(scifile0[i])):
                ...
                for q in range(0,len(throughput_array)):
                    if len(wavcal_array[q].wav)>0:
                        ax3.plot(wavcal_array[q].wav[throughputcorr_array[q].spec1d_mask==False],
                                 throughputcorr_array[q].spec1d_flux.value[throughputcorr_array[q].spec1d_mask==False],alpha=0.2,lw=0.3)
    ...
    if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
        ...
        for j in range(0,len(scifile0[i])):
            ...
            new_hdul = m2fs.get_hdul(data,skysubtract_array,sky_array,wavcal_array,plugmap0,m2fsrun,field_name[i],
                                             thar,[temperature[len(temperature)-1]])
...
def cheat_id_lines_translate(a):
    for j in range(0,len(tharfile0[i])):
        root0 = datadir+utdate[i]+'/'+ccd+str(tharfile0[i][j]).zfill(4)  # use first ThAr from first field in run
        extract1d_array_file = root0+'_extract1d_array.pickle'
        id_lines_array_file = root0+'_id_lines_array.pickle'
        ...
        if (not(cheat_id_lines_array_exist))|(cheat_id_lines_array_exist & overwrite):
            ...
            for j in range(0,len(extract1d_array)):
                ...
                if len(shite0)>0:
                    cheat_id_lines_array.append(m2fs.id_lines(aperture=id_lines_array[shite0[0]].aperture,
                                                              fit_lines=id_lines_array[shite0[0]].fit_lines,wav=id_lines_array[shite0[0]].wav,
                                                              func=id_lines_array[shite0[0]].func,rms=id_lines_array[shite0[0]].rms,
                                                              npoints=id_lines_array[shite0[0]].npoints,
                                                              resolution=id_lines_array[shite0[0]].resolution,
                                                              resolution_rms=id_lines_array[shite0[0]].resolution_rms,
                                                              resolution_npoints=id_lines_array[shite0[0]].resolution_npoints))
                else:
                    cheat_id_lines_array.append(m2fs.id_lines(aperture=extract1d_array[j].aperture,fit_lines=fit_lines,wav=wav,func=func,
                                                              rms=rms,npoints=npoints,resolution=resolution,resolution_rms=resolution_rms,
